...
describe('the library can produce and validate signatures', function() {
    it('should be able to generate a random private key', function() {
        ...
        const result = generate_private_key(private_key);
        assert(
            result == protos.ErrorCode.NO_ERROR,
    ...
    it('should be able to validate a valid signature', function() {
        ...
        assert(
            verified == protos.ErrorCode.NO_ERROR,
            'failed to validate a signature',
        );
    });
    ...
    it('should not be able to validate an invalid signature', async function() {
        ...
        assert(
            verified == protos.ErrorCode.SIGNATURE_VERIFICATION_FAILURE,
            'validated an incorrect signature',
        );
    });
    ...
    it('should not be able to validate with the incorrect public key', function() {
        ...
        assert(
            verified == protos.ErrorCode.INVALID_PUBLIC_KEY,
            'validation succeeded using an invalid public key',
        );
    });
    ...
    it('should not be able to validate with the incorrect private key', function() {
        ...
        assert(
            result == 3,
            'validation succeeded using an invalid private key',
        );
    });
    ...
    it('should not be able to validate with the incorrect context', function() {
        ...
        assert(
            result == protos.ErrorCode.INVALID_CONTEXT_LENGTH,
            'validation succeeded using an invalid context length',
        );
    });
    ...
    it('should be able to pass a known test vector', function() {
        ...
        assert(
            verified == protos.ErrorCode.NO_ERROR,
            'failed to validate using a known test vector',
        );
    });
});
...
describe('the library can batch verify signatures', function() {
    it('batch_verify_validates_multiple_correct_signatures', function() {
        ...
        const messages = [
            bytesFromString('Twas brillig, and the slithy toves'),
    ...
    it('batch verify fails on single incorrect message', function() {
        ...
        messages[messages.length - 1] = bytesFromString(
        ...
        assert(
            verified == protos.ErrorCode.BATCH_VERIFICATION_FAILURE,
            'batch verification should have failed',
        );
    });
